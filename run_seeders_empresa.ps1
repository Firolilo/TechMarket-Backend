...
$IA_SQL  = Join-Path $SCRIPT_DIR "TechMarket-IA\seed_empresa_test.sql"
$IAM_SQL = Join-Path $SCRIPT_DIR "TechMarket-IAM\seed_empresa_test.sql"
...
function Run-Seeder {
    ...
    docker cp $SqlFile "${Container}:/tmp/seed_empresa.sql"
    if ($LASTEXITCODE -ne 0) { Write-Host "[ERROR] No se pudo copiar el archivo." -ForegroundColor Red; exit 1 }

    docker exec -i $Container psql -U $User -d $DB -f /tmp/seed_empresa.sql
...
# ── Ejecutar ──────────────────────────────────────────────────
Run-Seeder `
    -Label     "TechMarket-IA  → seed_empresa_test.sql" `
    -Container $IA_CONTAINER  -DB $IA_DB  -User $IA_USER  `
    -SqlFile   $IA_SQL

Run-Seeder `
    -Label     "TechMarket-IAM → seed_empresa_test.sql" `
...
Write-Host "   Email:    empresa.test@techmarket.com" -ForegroundColor White
Write-Host "   Password: Empresa123!" -ForegroundColor White
Write-Host "============================================" -ForegroundColor Green
