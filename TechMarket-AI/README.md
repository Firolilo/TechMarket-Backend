...
- `com.techmarket.ai.domain.*` — modelos y políticas.
- `com.techmarket.ai.application.port.in` / `port.out` / `service` / `dto` — casos de uso y puertos.
- `com.techmarket.ai.infrastructure.*` — adapters (LLM, vector store, cache, etc.).
- `com.techmarket.ai.api.*` — controllers, request/response, error, OpenAPI.
- `com.techmarket.ai.bootstrap.*` — `@SpringBootApplication`, config, seguridad.
